...
Section "Oxford to Notion (required)" SecMain
    ...
    File "dist\${APP_EXE}"
    File /oname=Oxford-to-Notion-v1.4.7.ico "assets\app-icon.ico"
    WriteUninstaller "$INSTDIR\Uninstall.exe"

    CreateShortcut "$SMPROGRAMS\Oxford to Notion.lnk" "$INSTDIR\${APP_EXE}" "" "$INSTDIR\Oxford-to-Notion-v1.4.7.ico" 0
    IfFileExists "$DESKTOP\Oxford to Notion.lnk" 0 +2
    CreateShortcut "$DESKTOP\Oxford to Notion.lnk" "$INSTDIR\${APP_EXE}" "" "$INSTDIR\Oxford-to-Notion-v1.4.7.ico" 0
